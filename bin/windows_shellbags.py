...
if hosts:
	r2 = re.compile(".*shellbags.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Registry", "Key", "LastWrite", "FileName", "Create", "Access", "Attributes", "Unicode", "Path")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_shellbags = None
					try:
						windows_shellbags =  json.loads(f.readline())
					except:
						pass

					if windows_shellbags:
						for element in windows_shellbags['rows']:
