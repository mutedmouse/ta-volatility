...
if hosts:
	r2 = re.compile(".*lsadump.*")
	r3 = re.compile("(?!.*secrets).*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}".format("host", "Item", "Data")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_lsadump = None
						try:
							windows_lsadump =  json.loads(f.readline())
						except:
							pass
	
						if windows_lsadump:
							for element in windows_lsadump['rows']:
