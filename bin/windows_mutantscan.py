	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Offset(P)", "Pointers", "Handles", "Signal", "Thread", "CID", "Name")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_mutantscan = None
						try:
							windows_mutantscan =  json.loads(f.readline())
						except:
							pass
	
						if windows_mutantscan:
							for element in windows_mutantscan['rows']:
