	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Session", "WindowStation", "Format", "Handle", "Object", "Data")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_clipboard = None
						try:
							windows_clipboard = json.loads(f.readline())
						except:
							pass
	
						if windows_clipboard:
							for element in windows_clipboard['rows']:
