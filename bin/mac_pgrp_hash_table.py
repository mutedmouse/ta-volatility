...
if hosts:
	r2 = re.compile(".*mac_pgrp_hash_table.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Offset", "Name", "PID", "UID", "GID", "PGID", "Bits", "DTB", "Start Time")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					mac_pgrp_hash_table = None
					try:
						mac_pgrp_hash_table =  json.loads(f.readline())
					except:
						pass
					
					if mac_pgrp_hash_table:
						for element in mac_pgrp_hash_table['rows']:
