	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}".format("Symbol Name", "Address", "Symbol")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						linux_check_tty = None
						try:
							linux_check_tty =  json.loads(f.readline())
						except:
							pass
						
						if linux_check_tty:
							for element in linux_check_tty['rows']:
