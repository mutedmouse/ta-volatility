...
if hosts:
	r2 = re.compile(".*cmdscan.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", Process", "PID", "History Offset", "Application", "Flags", "Command Count", "Last Added", "Last Displayed", "First Command", "Command Count Max", "Handle", "Command Number", "Command Offset", "Command")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_cmdscan = None
					try:
						windows_cmdscan =  json.loads(f.readline())
					except:
						pass

					if windows_cmdscan:
						for element in windows_cmdscan['rows']:
