...
if hosts:
	r2 = re.compile(".*amcache.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Registry", "Key Path", "Key Last updated", "Value Name", "Description", "Value")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_amcache = None
					try:
						windows_amcache =  json.loads(f.readline())
					except:
						pass

					if windows_amcache:
						for element in windows_amcache['rows']:
