	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}".format("host", "Virtual", "Physical", "Name")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_hivelist = None
						try:
							windows_hivelist =  json.loads(f.readline())
						except:
							pass
	
						if windows_hivelist:
							for element in windows_hivelist['rows']:
