	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("image","tag","npallocs","npfrees","npbytes","pgallocs","pgfrees","pgbytes","driver","reason")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_pooltracker = None
						try:
							windows_pooltracker =  json.loads(f.readline())
						except:
							pass
	
						if windows_pooltracker:
							for element in windows_pooltracker['rows']: 
