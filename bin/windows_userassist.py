...
if hosts:
	r2 = re.compile(".*userassist.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Registry", "Path", "LastWrite", "Subkey", "Value", "ID", "Count", "FocusCount", "TimeFocused", "LastUpdated")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_userassist = None
					try:
						windows_userassist =  json.loads(f.readline())
					except:
						pass

					if windows_userassist:
						for element in windows_userassist['rows']:
