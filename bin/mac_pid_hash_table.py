...
if hosts:
	r2 = re.compile(".*mac_pid_hash_table.*")
			...
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Offset", "Name", "PID", "UID", "GID", "PGID", "Bits", "DTB", "Start Time")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						mac_pid_hash_table = None
						try:
							mac_pid_hash_table =  json.loads(f.readline())
						except:
							pass
						
						if mac_pid_hash_table:
							for element in mac_pid_hash_table['rows']:
