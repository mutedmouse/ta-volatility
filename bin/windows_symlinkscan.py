...
if hosts:
	r2 = re.compile(".*symlinkscan.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Offset(P)", "Pointers", "Handles", "Creation Time", "Origin", "Target")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_symlinkscan = None
					try:
						windows_symlinkscan =  json.loads(f.readline())
					except:
						pass

					if windows_symlinkscan:
						for element in windows_symlinkscan['rows']:
