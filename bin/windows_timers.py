...
if hosts:
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", Offset(V)", "DueTime", "Period(ms)", "Signaled", "Routine", "Module")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_timers = None
					try:
						windows_timers =  json.loads(f.readline())
					except:
						pass


					if windows_timers:
						for element in windows_timers['rows']:
