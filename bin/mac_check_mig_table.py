...
if hosts:
	r2 = re.compile(".*mac_check_mig_table.*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Index", "Routine Name", "Routine Handler")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						mac_check_mig_table = None
						try:
							mac_check_mig_table =  json.loads(f.readline())
						except:
							pass
						
						if mac_check_mig_table:
							for element in mac_check_mig_table['rows']:
