...
if hosts:
	r2 = re.compile(".*linux_proc_maps_rb.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}".format("Offset", "Pid", "Name", "Start", "End", "Flags", "Pgoff", "Major", "Minor", "Inode", "Path")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					linux_proc_maps_rb = None
					try:
						linux_proc_maps_rb =  json.loads(f.readline())
					except:
						pass
					
					if linux_proc_maps_rb:
						for element in linux_proc_maps_rb['rows']:
