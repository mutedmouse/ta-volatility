	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Pid", "Process", "Base", "InLoad", "InInit", "InMem", "MappedPath")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_ldrmodules = None
						try:
							windows_ldrmodules =  json.loads(f.readline())
						except:
							pass
	
						if windows_ldrmodules:
							for element in windows_ldrmodules['rows']:
