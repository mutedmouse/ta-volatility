	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format(host,"Process", "Pid", "Address", "VadTag", "Protection", "Flags", "Data")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_malfind = None
						try:
							windows_malfind =  json.loads(f.readline())
						except:
							pass
	
						if windows_malfind:
							for element in windows_malfind['rows']:
