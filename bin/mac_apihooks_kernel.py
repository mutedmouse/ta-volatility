...
if hosts:
	r2 = re.compile(".*mac_apihooks_kernel.*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Table Name", "Index", "Address", "Symbol", "Inlined", "Shadowed", "Perms", "Hook In")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						mac_apihooks_kernel = None
						try:
							mac_apihooks_kernel =  json.loads(f.readline())
						except:
							pass
						
						if mac_apihooks_kernel:
							for element in mac_apihooks_kernel['rows']:
