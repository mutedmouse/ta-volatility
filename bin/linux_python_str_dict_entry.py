...
if hosts:
	r2 = re.compile(".*linux_python_str_dict_entry.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}".format("Pid", "Name", "Key", "Value")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					linux_python_str_dict_entry = None
					try:
						linux_python_str_dict_entry =  json.loads(f.readline())
					except:
						pass
					
					if linux_python_str_dict_entry:
						for element in linux_python_str_dict_entry['rows']:
