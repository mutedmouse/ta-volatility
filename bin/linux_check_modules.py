...
if hosts:
	r2 = re.compile(".*linux_check_modules.*")
	for host in hosts:
		...
		#print "{}".format("Module Name")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					linux_check_modules = None
					try:
						linux_check_modules =  json.loads(f.readline())
					except:
						pass
					
					if linux_check_modules:
						for element in linux_check_modules['rows']:
