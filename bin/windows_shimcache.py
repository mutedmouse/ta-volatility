	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}".format("host", "Last Modified", "Last Update", "Path")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_shimcache = None
						try:
							windows_shimcache =  json.loads(f.readline())
						except:
							pass
	
						if windows_shimcache:
							for element in windows_shimcache['rows']:
