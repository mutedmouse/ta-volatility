	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}".format("host", "Module Base", "Module Name", "Result")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_moddump = None
						try:
							windows_moddump =  json.loads(f.readline())
						except:
							pass
	
						if windows_moddump:
							for element in windows_moddump['rows']:
