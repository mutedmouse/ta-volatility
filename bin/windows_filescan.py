...
if hosts:
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				temp = ""
				#print "{}\t{}\t{}\t{}\t{}\t{}".format(host, "Offset(P)", "Pointers", "Handles", "Access", "Name"])
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_filescan = None
					try:
						windows_filescan =  json.loads(f.readline())
					except:
						pass

					if windows_filescan:
						for element in windows_filescan['rows']:
