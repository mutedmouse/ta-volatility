...
if hosts:
	r2 = re.compile(".*mac_socket_filters.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Offset (V)", "Filter Name", "Filter Member", "Socket (V)", "Handler", "Module", "Status")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					mac_socket_filters = None
					try:
						mac_socket_filters =  json.loads(f.readline())
					except:
						pass
					
					if mac_socket_filters:
						for element in mac_socket_filters['rows']:
