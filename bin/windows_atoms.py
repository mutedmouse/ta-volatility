	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print {}\t{}\t{}\t{}\t{}".format("host", "Offset(V)", "Session", "WindowStation", "Atom", "RefCount", "HIndex", "Pinned", "Name")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_atoms = None
						try:
							windows_atoms =  json.loads(f.readline())
						except:
							pass
	
						if windows_atoms:
							for element in windows_atoms['rows']:
