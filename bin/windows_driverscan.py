	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", Offset(P)", "Pointers", "Handles", "Start", "Size", "Service Key", "Name", "Driver Name")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_driverscan = None
						try:
							windows_driverscan =  json.loads(f.readline())
						except:
							pass
	
						if windows_driverscan:
							for element in windows_driverscan['rows']:
