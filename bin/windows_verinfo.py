	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Module", "FileVersion", "ProductVersion", "Flags", "OS", "FileType", "FileDate", "InfoString")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_verinfo = None
						try:
							windows_verinfo =  json.loads(f.readline())
						except:
							pass
	
						if windows_verinfo:
							for element in windows_verinfo['rows']:
								element = [str(x) for x in element]
								element = [(re.sub(r'\\', r'\\\\', str(el))) for el in element]
								element = [(re.sub(r'\x00', r'', str(el))) for el in element]
