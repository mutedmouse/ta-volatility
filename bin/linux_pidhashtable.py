...
if hosts:
	r2 = re.compile(".*linux_pidhashtable.*")
			...
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Offset", "Name", "PID", "UID", "GID", "DTB", "Start Time")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						linux_pidhashtable = None
						try:
							linux_pidhashtable =  json.loads(f.readline())
						except:
							pass
						
						if linux_pidhashtable:
							for element in linux_pidhashtable['rows']:
