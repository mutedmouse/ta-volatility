...
if hosts:
	r2 = re.compile(".*mac_print_boot_cmdline.*")
			...
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Command Line")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						mac_print_boot_cmdline = None
						try:
							mac_print_boot_cmdline =  json.loads(f.readline())
						except:
							pass
						
						if mac_print_boot_cmdline:
							for element in mac_print_boot_cmdline['rows']:
