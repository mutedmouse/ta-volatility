...
if hosts:
	r2 = re.compile(".*mac_lsmod_kext_map.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Offset (V)", "Module Address", "Size", "Refs", "Version", "Name")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					mac_lsmod_kext_map = None
					try:
						mac_lsmod_kext_map =  json.loads(f.readline())
					except:
						pass
					
					if mac_lsmod_kext_map:
						for element in mac_lsmod_kext_map['rows']:
