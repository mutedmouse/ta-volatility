	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Offset(V)", "PID", "Port", "Proto", "Protocol", "Address", "Create Time")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_sockscan = None
						try:
							windows_sockscan =  json.loads(f.readline())
						except:
							pass
	
						if windows_sockscan:
							for element in windows_sockscan['rows']:
