	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("image","asciichar","asciicode","scancode")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_bioskbd = None
						try:
							windows_bioskbd =  json.loads(f.readline())
						except:
							pass
	
						if windows_bioskbd:
							for element in windows_bioskbd['rows']: 
