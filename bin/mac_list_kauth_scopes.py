...
if hosts:
	r2 = re.compile(".*mac_list_kauth_scopes.*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Offset", "Name", "IData", "Listeners", "Callback Addr", "Callback Mod", "Callback Sym")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						mac_list_kauth_scopes = None
						try:
							mac_list_kauth_scopes =  json.loads(f.readline())
						except:
							pass
						
						if mac_list_kauth_scopes:
							for element in mac_list_kauth_scopes['rows']:
