	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("host", "Offset(V)", "Name", "Base", "Size", "File")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_timeliner = None
						try:
							windows_timeliner =  json.loads(f.readline())
						except:
							pass
	
						if windows_timeliner:
							for element in windows_timeliner['rows']: 
