...
if hosts:
	r2 = re.compile(".*linux_check_inline_kernel.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}".format("Name", "Member", "HookType", "HookAddress")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					linux_check_inline_kernel = None
					try:
						linux_check_inline_kernel =  json.loads(f.readline())
					except:
						pass
					
					if linux_check_inline_kernel:
						for element in linux_check_inline_kernel['rows']:
