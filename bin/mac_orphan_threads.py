...
if hosts:
	r2 = re.compile(".*mac_orphan_threads.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("PID", "Process Name", "Start Address", "Mapping", "Name", "Status")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					mac_orphan_threads = None
					try:
						mac_orphan_threads =  json.loads(f.readline())
					except:
						pass
					
					if mac_orphan_threads:
						for element in mac_orphan_threads['rows']:
