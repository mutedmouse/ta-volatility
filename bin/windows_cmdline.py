	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("host", "Process", "PID", "Command Line")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_cmdline = None
						try:
							windows_cmdline =  json.loads(f.readline())
						except:
							pass
	
						if windows_cmdline:
							for element in windows_cmdline['rows']: 
