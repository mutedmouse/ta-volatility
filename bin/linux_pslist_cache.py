...
if hosts:
	r2 = re.compile(".*linux_pslist_cache.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Offset", "Name", "PID", "UID", "GID", "DTB", "Start Time")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					linux_pslist_cache = None
					try:
						linux_pslist_cache =  json.loads(f.readline())
					except:
						pass
					
					if linux_pslist_cache:
						for element in linux_pslist_cache['rows']:
