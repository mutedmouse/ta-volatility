	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}".format("host", "SID", "Service"])
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_getservicesids = None
						try:
							windows_getservicesids =  json.loads(f.readline())
						except:
							pass
	
						if windows_getservicesids:
							for element in windows_getservicesids['rows']:
