...
if hosts:
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Offset(V)", "Pid", "Handle", "Access", "Type", "Details")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_handles = None
					try:
						windows_handles =  json.loads(f.readline())
					except:
						pass

					if windows_handles:
						for element in windows_handles['rows']:
