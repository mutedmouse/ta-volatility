	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("image","registry","keyname","keystability","lastwrite","subkeys","subkeystability","valtype","valname","valstability","valdata")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_printkey = None
						try:
							windows_printkey =  json.loads(f.readline())
						except:
							pass
	
						if windows_printkey:
							for element in windows_printkey['rows']: 
