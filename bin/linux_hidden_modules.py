...
if hosts:
	r2 = re.compile(".*linux_hidden_modules.*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}".format("Offset(V)", "Name")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						linux_hidden_modules = None
						try:
							linux_hidden_modules =  json.loads(f.readline())
						except:
							pass
						
						if linux_hidden_modules:
							for element in linux_hidden_modules['rows']:
