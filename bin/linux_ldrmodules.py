...
if hosts:
	r2 = re.compile(".*linux_ldrmodules.*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}".format("Pid", "Name", "Start", "Path", "Kernel", "Libc")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						linux_ldrmodules = None
						try:
							linux_ldrmodules =  json.loads(f.readline())
						except:
							pass
						
						if linux_ldrmodules:
							for element in linux_ldrmodules['rows']:
