...
if hosts:
	r2 = re.compile(".*mac_list_zones.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}".format("Name", "Active Count", "Free Count", "Element Size")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					mac_list_sessions = None
					try:
						mac_list_sessions =  json.loads(f.readline())
					except:
						pass
					
					if mac_list_sessions:
						for element in mac_list_sessions['rows']:
