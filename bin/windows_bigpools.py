	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("image","allocation","tag","pooltype","numberofbytes")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_bigpools = None
						try:
							windows_bigpools =  json.loads(f.readline())
						except:
							pass
	
						if windows_bigpools:
							for element in windows_bigpools['rows']: 
