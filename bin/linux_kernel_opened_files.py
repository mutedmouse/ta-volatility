...
if hosts:
	r2 = re.compile(".*linux_kernel_opened_files.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
		#print "{}\t{}\t{}\t{}".format("Offset (V)", "Partial File Path")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					linux_kernel_opened_files = None
					try:
						linux_kernel_opened_files =  json.loads(f.readline())
					except:
						pass
					
					if linux_kernel_opened_files:
						for element in linux_kernel_opened_files['rows']:
