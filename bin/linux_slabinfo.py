	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Name", "Active Objects", "Number Objects", "Object Size", "Objects Per Slab", "Pages Per Slab", "Active Slabs", "Number Slabs")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						linux_slabinfo = None
						try:
							linux_slabinfo =  json.loads(f.readline())
						except:
							pass
						
						if linux_slabinfo:
							for element in linux_slabinfo['rows']:
