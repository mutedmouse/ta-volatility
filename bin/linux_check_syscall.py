...
if hosts:
	r2 = re.compile(".*linux_check_syscall.*")
	for host in hosts:
		...
		#print "{}\t{}\t{}\t{}".format("Table Name", "Index", "Address", "Symbol")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					linux_check_syscall = None
					try:
						linux_check_syscall =  json.loads(f.readline())
					except:
						pass
					
					if linux_check_syscall:
						for element in linux_check_syscall['rows']:
