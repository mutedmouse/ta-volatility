...
if hosts:
	r2 = re.compile(".*linux_truecrypt_passphrase.*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}".format("Offset", "Name", "Pid", "Uid", "Gid", "DTB", "StartTime")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						linux_truecrypt_passphrase = None
						try:
							linux_truecrypt_passphrase =  json.loads(f.readline())
						except:
							pass
						
						if linux_truecrypt_passphrase:
							for element in linux_truecrypt_passphrase['rows']:
