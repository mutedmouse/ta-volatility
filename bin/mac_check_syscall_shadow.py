...
if hosts:
	r2 = re.compile(".*mac_check_syscall_shadow.*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("Hooked Function", "Hook Address", "Instruction")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						mac_check_syscall_shadow = None
						try:
							mac_check_syscall_shadow =  json.loads(f.readline())
						except:
							pass
						
						if mac_check_syscall_shadow:
							for element in mac_check_syscall_shadow['rows']:
