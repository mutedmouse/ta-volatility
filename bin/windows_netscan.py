...
if hosts:
	r2 = re.compile(".*netscan.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
				#print "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format("host", "Offset(P)", "Proto", "LocalAddr", "ForeignAddr", "State", "PID", "Owner", "Created")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_netscan = None
					try:
						windows_netscan =  json.loads(f.readline())
					except:
						pass

					if windows_netscan:
						for element in windows_netscan['rows']:
