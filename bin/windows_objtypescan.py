...
if hosts:
	r2 = re.compile(".*objtypescan.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("image","offset","nobjects","nhandles","key","name","pooltype")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_objtypescan = None
					try:
						windows_objtypescan =  json.loads(f.readline())
					except:
						pass

					if windows_objtypescan:
						for element in windows_objtypescan['rows']: 
