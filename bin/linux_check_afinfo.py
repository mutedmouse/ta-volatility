...
if hosts:
	r2 = re.compile(".*linux_check_afinfo.*")
	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}".format("Symbol Name", "Member", "Address")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						linux_check_afinfo = None
						try:
							linux_check_afinfo =  json.loads(f.readline())
						except:
							pass
						
						if linux_check_afinfo:
							for element in linux_check_afinfo['rows']:
