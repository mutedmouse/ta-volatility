	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}".format("host", "Name", "StartAddress", "EndAddress", "Time")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_unloadedmodules = None
						try:
							windows_unloadedmodules =  json.loads(f.readline())
						except:
							pass
	
						if windows_unloadedmodules:
							for element in windows_unloadedmodules['rows']:
