	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}".format("host", "Type", "Callback", "Module", "Details")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_callbacks = None
						try:
							windows_callbacks =  json.loads(f.readline())
						except:
							pass
	
						if windows_callbacks:
							for element in windows_callbacks['rows']:
