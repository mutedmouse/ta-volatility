import sys, json, os, re, platform, itertools, importlib, TAplugins
...
class parser:
	...
	# it then checks imported TAplugins classes to verify plugin by string
	...
	def parse(self, file_sep, base_dir, builderDict):
		#prepare import of TAplugins.py classes
		...
		for arch in builderDict.keys():
			...
			for plugin in builderDict[arch].keys():
				...
				#TAplugins via the pluginmodule that loaded the module earlier
				#
				...
				if instance:
					...
					#
					#If using custom formatting in TAplugins.py please
	...
	def format(self, file_sep, base_dir, arch, plugin, host):
		...
		if container:
			...
			for element in container['rows']:
				...
				temp={}
			#Upon completion with the current file, delete it to prevent ingest on next
			# run
