		...
		if target:
			for fitem in target:
				if os.stat(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem)).st_size != 0:
				 #print "{}\t{}\t{}\t{}\t{}\t{}".format(host, "Offset(P)", "Name", "Base", "Size", "File")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_modscan = None
						try:
							windows_modscan =  json.loads(f.readline())
						except:
							pass
	
						if windows_modscan:
							for element in windows_modscan['rows']:
