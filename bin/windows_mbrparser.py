	...
	for host in hosts:
		...
		if target:
			for fitem in target:
				...
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("image","offset","disksignature","bootmd5","fullbootmd5","distance","partabootflag","partatype","partalba","partastartchs","partaendchs","partasectorsize","partbbootflag","partbtype","partblba","partbstartchs","partbendchs","partbsectorsize","partcbootflag","partctype","partclba","partcstartchs","partcendchs","partcsectorsize","partdbootflag","partdtype","partdlba","partdstartchs","partdendchs","partdsectorsize","bootcode")
					with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
						windows_mbrparser = None
						try:
							windows_mbrparser =  json.loads(f.readline())
						except:
							pass
	
						if windows_mbrparser:
							for element in windows_mbrparser['rows']: 
