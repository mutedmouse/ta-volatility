...
if hosts:
	r2 = re.compile(".*iehistory.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
				#print "{}\t{}\t{}\t{}\t{}\t{}".format("image","process","pid","cachetype","offset","recordlength","location","lastmodified","lastaccessed","length","fileoffset","dataoffset","datasize","file","data")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_iehistory = None
					try:
						windows_iehistory =  json.loads(f.readline())
					except:
						pass

					if windows_iehistory:
						for element in windows_iehistory['rows']: 
