...
if hosts:
	r2 = re.compile(".*vadinfo.*")
	for host in hosts:
		targets = os.listdir(str(base_dir)+str(platform_target[0])+file_sep+str(host))
		target = filter(r2.match, targets)
		if target:
			for fitem in target:
				#print "{}\t{}\t{}\t{}\t{}\t{}".format(image","pid","vadnodeaddress","start","end","tag","flags","protection","vadtype","controlarea","segment","numberofsectionreferences","numberofpfnreferences","numberofmappedviews","numberofuserreferences","control_flags","fileobject","filenamewithdevice","firstprototypepte","lastcontiguouspte","flags2")
				with open(str(str(base_dir)+str(platform_target[0])+file_sep+str(host)+file_sep+fitem), 'r') as f:
					windows_vadinfo = None
					try:
						windows_vadinfo =  json.loads(f.readline())
					except:
						pass

					if windows_vadinfo:
						for element in windows_vadinfo['rows']: 
